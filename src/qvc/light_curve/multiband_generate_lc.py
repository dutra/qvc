...
def concat_light_curves(filter_object_ids=None, progress_bar=False, skip=None, N=None):
    ...
    hdul = fits.open(resolve_qvc_data_path("data/dr16q_prop_May01_2024.fits"))
    fits_data = hdul[1].data  # kept in case you use it elsewhere
    ...
    print(f"Found {len(cat)} matching objects in concat_light_curves")
    ...
    for idx, row in tqdm(
        ...
        for band in bands:
            ...
            t_band = np.concatenate([
                sdss_lc.loc[sdss_lc.filterID == filters[band], "mjd"].values if not sdss_lc.empty else np.array([]),
                ps1_lc.loc[ps1_lc.filterID == filters[band], "obsTime"].values if not ps1_lc.empty else np.array([]),
                ztf_lc.loc[ztf_lc.filterID == filters[band], "mjd"].values if not ztf_lc.empty else np.array([]),
            ])

            m_band = np.concatenate([
                sdss_lc.loc[sdss_lc.filterID == filters[band], "psMag"].values if not sdss_lc.empty else np.array([]),
                ps1_lc.loc[ps1_lc.filterID == filters[band], "psfMag"].values + offset if not ps1_lc.empty else np.array([]),
                ztf_lc.loc[ztf_lc.filterID == filters[band], "mag"].values + offset if not ztf_lc.empty else np.array([]),
            ])

            me_band = np.concatenate([
                sdss_lc.loc[sdss_lc.filterID == filters[band], "psMagErr_p3"].values if not sdss_lc.empty else np.array([]),
                ps1_lc.loc[ps1_lc.filterID == filters[band], "psfMagErr_p3"].values if not ps1_lc.empty else np.array([]),
                ztf_lc.loc[ztf_lc.filterID == filters[band], "magerr_p3"].values if not ztf_lc.empty else np.array([]),
            ])

            # Drop NaNs using the same mask
            ...
            if len(t_band) == 0:
                times[band] = np.array([])
                mags[band] = np.array([])
                magerrs[band] = np.array([])
                continue

            sort_idx = np.argsort(t_band)
            times[band] = t_band[sort_idx]
            mags[band] = m_band[sort_idx]
            magerrs[band] = me_band[sort_idx]

        # Mean magnitude per band
        mags_means = [
            np.nanmean(mags[band]) if len(mags[band]) > 0 else np.nan
            for band in bands
        ]

        # New: all times per survey, combining all bands
        survey_times = {
            "sdss": _clean_sort(sdss_lc["mjd"].values) if not sdss_lc.empty else np.array([]),
            "ps1": _clean_sort(ps1_lc["obsTime"].values) if not ps1_lc.empty else np.array([]),
            "ztf": _clean_sort(ztf_lc["mjd"].values) if not ztf_lc.empty else np.array([]),
        }

        # Skip if no data is available for the object
        if all(len(times[band]) == 0 for band in bands):
            print(f"No data available for object {object_id}, skipping.", flush=True)
            continue

        # Gather epochs across all bands
        all_times = np.concatenate([np.asarray(times[band], float) for band in bands])
        all_times = all_times[np.isfinite(all_times)]
        all_times = np.sort(all_times)

        # Collapse near-simultaneous visits
        tol_min = 30
        ...
        # Nearest-neighbour spacings; drop huge seasonal gaps
        gap_days = 30
        if len(all_times) > 1:
    ...
    print(f"Found {len(s82_objs)} objects in concat_light_curves after time cut")

    return s82_objs

def concat_light_curves_jax(filter_object_ids=None, progress_bar=False, skip=None, N=None):
    """Vectorized light-curve concatenation with the same public schema as concat_light_curves."""
    print(
        f"[DEBUG] Loading concat_light_curves_jax with filter_object_ids={filter_object_ids}, skip={skip}, N={N}"
    )
