...
`modified_tight` is the default eta prior profile for both the CLI and Python API. It uses underlying Normal location/scale (-0.8, 0.25) truncated to [-1.5, 0] for eta_sigma, and (0.5, 0.25) truncated to [0, 1.5] for eta_tau in models with wavelength-dependent drivers. Original SLB has no eta_tau. To reproduce the previous default, pass `--eta-prior-profile relaxed`. The old profile name `default` is no longer accepted; `modified` remains available and unchanged. Existing saved artifacts are not rewritten.
