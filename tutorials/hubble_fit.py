...
def get_completeness_function_2d(df_agn, sim_file="sampled_apparent_magnitudes_redshift.h5",
    ...
    interp_fn = RegularGridInterpolator(
        (mag_centers, z_centers), completeness_ratio.T,  # .T for (mag, z) order
        bounds_error=False, fill_value=0.0
    )

    def p_detect(mag, z):
        """
        Evaluate completeness at given mag and z (can be arrays).
        """
    ...
    return p_detect, mag_centers, z_centers, dm, dz
...
def plot_hubble(sampler, df_agn, df_pantheon, cosmo_model, show=False):
    ...
    mu_pred = np.array([
        df_agn['apparent_mag_i'] - K_corr(df_agn['z']) - (M_model_single(s[1], s[0], df_agn['log_sigma_UV'], df_agn['log_tau_UV_RF']) - K_corr(2))
...
def plot_predicted_vs_actual_Mi(sampler, df_agn, cosmo_model, show=False):
    ...
    # --- Predicted M_i ---
    M_pred = np.array([
        M_model_single(s[1], s[0], df_agn['log_sigma_UV'], df_agn['log_tau_UV_RF']) + K_corr(2) # TODO: check this
        for s in flat_samples
    ]) 
