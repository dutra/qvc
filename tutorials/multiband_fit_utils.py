...
def estimate_numpyro_gpu_memory_vectorized(
    num_params_per_object: int,
    num_objects: int,
    num_chains: int,
    num_samples: int,
    num_warmup: int,
    ...
    model_overhead_mb: float = 1024.0,
) -> float:
    """
    Estimate GPU memory usage for chain_method='vectorized'.
    """
    total_params = num_params_per_object * num_objects
    total_states = num_chains * total_params
    state_memory = total_states * dtype_bytes

    # Cached samples and diagnostics (~10 samples per chain)
    sample_cache = num_chains * total_params * 10 * dtype_bytes

    total_mem_bytes = (state_memory + sample_cache) * buffer_factor
    total_mem_bytes += model_overhead_mb * 1024**2

    return total_mem_bytes / 1024**3

def estimate_numpyro_gpu_memory_parallel(
    num_params_per_object: int,
    num_objects: int,
    num_chains: int,
    dtype_bytes: int = 8,
    cache_per_chain: int = 10,
    buffer_factor: float = 2.0,
    model_overhead_mb: float = 512.0,
) -> float:
    """
    Estimate GPU memory usage for chain_method='parallel'.
    """
    total_params = num_params_per_object * num_objects
    state_mem = total_params * dtype_bytes
    cache_mem = total_params * cache_per_chain * dtype_bytes

    total_mem_bytes = num_chains * (state_mem + cache_mem) * buffer_factor
    total_mem_bytes += model_overhead_mb * 1024**2

    return total_mem_bytes / 1024**3

def estimate_numpyro_gpu_memory_sequential(
    num_params_per_object: int,
    num_objects: int,
    dtype_bytes: int = 8,
    buffer_factor: float = 2.0,
    model_overhead_mb: float = 512.0,
    working_buffer_mb: float = 256.0,
) -> float:
    """
    Estimate GPU memory usage for chain_method='sequential'.
    """
    total_params = num_params_per_object * num_objects
    state_mem = total_params * dtype_bytes * buffer_factor

    total_mem_bytes = state_mem + (model_overhead_mb + working_buffer_mb) * 1024**2
    return total_mem_bytes / 1024**3
