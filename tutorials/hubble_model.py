...
def M_model_agn(M0_agn, log_sigma_hat_sq_break, eta_A1_agn, eta_A2_agn, eta_break_agn, beta_agn, gamma_agn, log_sigma_hat_UV, log_tau_UV_RF, alpha_nu):
    ...
    return M0_agn + bpl + beta_agn * (log_tau_UV_RF - log_tau_UV_RF_pivot) + gamma_agn * (alpha_nu - alpha_nu_pivot)

# keep this same(ish) signature as M_model_agn + x_err
def M_model_agn_err(M0_agn, log_sigma_hat_sq_break, eta_A1_agn, eta_A2_agn, eta_break_agn, beta_agn, gamma_agn,
                    log_sigma_hat_UV, log_sigma_hat_UV_err, log_tau_UV_RF_err, alpha_nu_err):
    ...
    return np.sqrt(err_bpl**2 + (beta_agn * log_tau_UV_RF_err)**2 + (gamma_agn * alpha_nu_err)**2)
...
def get_model_params(cosmo_model):
    ...
    #     ("delta_M0_agn", (-6, 0)),         # M0_agn = M0_sn + delta_M0, MLE: ~-5.179
    #     ("log_sigma_hat_sq_break", (-1.2, -1.1)), # AGN broken power law break point
    #     ("eta_A1_agn",  (4, 6)),          # AGN broken power law slope 1
    #     ("eta_A2_agn",  (-0.2, 0.2)),          # AGN broken power law slope 2
    #     ("eta_break_agn", (1.8, 1.9)),      # AGN broken power law slope transition
    ...
    priors = OrderedDict([
        ...
        ("tau_Ms",      (0.01, 0.2)),     # LOG Width of sigmoid transition usually ~0.043
        ("M0_sn",       (-20, -19)),    # SN absolute magnitude, MLE: ~-19.3
        ("M0_agn", (-30, -20)),         # M0_agn = M0_sn + delta_M0, MLE: ~-5.179
        ("log_sigma_hat_sq_break", (-1.6, -0.8)), # AGN broken power law break point
        ("eta_A1_agn",  (-10, 10)),          # AGN broken power law slope 1
        ("eta_A2_agn",  (-10, 10)),          # AGN broken power law slope 2
        ("eta_break_agn", (0.5, 10)),      # AGN broken power law slope transition
        ("beta_agn",    (-3, 3)),         # AGN tau correlation
        ("gamma_agn",   (-1, 1)),         # AGN alpha_nu correlation
        ("log_f",       (-1.5, 0.5)),
        ("H0",          (65, 80)),
        #("Om0",         (0.32, 0.324)),
        ("Om0",         (0.2, 0.7)),
