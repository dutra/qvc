...
def _pos(x, eps=1e-12):
    # Smooth positivity without kinks (softplus is C∞)
    return jax.nn.softplus(x) + eps

def _safe_div(num, den, eps=1e-12):
    return num / (den + eps)

def _one_minus_exp_over_x(x):
    # Smooth, stable (1 - exp(-x)) / x for all x≥0
    # 4th-order series is plenty; avoids jnp.where branch.
    t = x
    s = 1.0 - 0.5*t + (t*t)/6.0 - (t**3)/24.0 + (t**4)/120.0
    return jnp.where(t < 1e-3, s, -jnp.expm1(-t) / t)
...
class MultiExp(qs.Quasisep):
    ...
    def design_matrix(self):
        lam = 1.0 / (self.tau + 1e-12)          # smooth, no max
        return -jnp.diag(lam)

    def stationary_covariance(self):
        ti = self.tau[:, None]
        tj = self.tau[None, :]
        P = 2.0 * jnp.sqrt(ti * tj) / (ti + tj + 1e-12)  # smooth denom
        return 0.5 * (P + P.T)

    def transition_matrix(self, t1, t2):
        dt = t2 - t1
        lam = 1.0 / (self.tau + 1e-12)
...
class ContiBLR_QS(qs.Wrapper):
    ...
    def __init__(self, amp_cont, amp_blr, lag_blr, lag_disk,
        ...
        eps = 1e-12
        # All inputs already positive from caller; keep them smooth & >0
        ...
        self.s          = jnp.asarray(s) + eps

        tau_ref  = jnp.asarray(tau_drw) + eps
        tau_band = _safe_div(tau_ref, self.s)       # smooth, positive
        self.kernel = MultiExp(tau=tau_band)

    def coord_to_sortable(self, X):
        t, b = X
        return t + 1e-9 * jnp.asarray(b, jnp.float32)  # keep purely float

    @staticmethod
    def _gain_top_hat(width, tau):
        x = _safe_div(width, tau)            # x = width / tau
        return _one_minus_exp_over_x(jnp.maximum(x, 0.0))  # x≥0 by model design
    ...
    def _conv_obs(self, band_idx, lag, width):
        ...
        h_lag = h0 @ Phi
        tau_b = self.kernel.tau[band_idx]
        G     = self._gain_top_hat(width, tau_b)
        return h_lag * G
