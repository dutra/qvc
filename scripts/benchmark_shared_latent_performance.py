...
def main():
    ...
    parser.add_argument("--variant", choices=["slb"], default="slb")
    parser.add_argument("--stage", choices=["likelihood", "training", "prediction", "plot", "loo"], default="likelihood")
    ...
    theta = jnp.array([np.log(2.), np.log(300.),
                      np.log(3.), np.log(100.), np.log(.1), .5])

    def parameters(p):
        f, s, lag, blr, amp = jnp.exp(p[:5])
        return dict(tau_fast_driver=f, tau_slow_driver=s,
